 ...
 * Event Ticket & Scoreboard Card Component (Perforated Stub Edition)
 */
function createNextPlayTicket(event) {
    ...
    const actionBtn = isCompleted
        ? `<a href="${event.locationUrl}" target="_blank" rel="noopener noreferrer" class="btn" style="white-space:nowrap; background:#C0392B; color:#fff; border:2px solid #7B1A13; box-shadow:4px 4px 0 #7B1A13;">
               <span>LIHAT LOKASI</span>
               <span style="font-family:var(--font-pixel);">↗</span>
           </a>`
        : `<a href="${event.locationUrl}" target="_blank" rel="noopener noreferrer" class="btn btn-primary" style="white-space:nowrap; box-shadow:4px 4px 0px var(--color-dark);">
               <span>PETUNJUK LOKASI</span>
               <span style="font-family:var(--font-pixel);">↗</span>
           </a>`;
    const typeBadge = isCompleted
        ? `<span class="badge" style="background:#FDDDD9; color:#7B1A13; border:1.5px solid #C0392B;">${event.typeName}</span>`
        : `<span class="badge badge--dark-green">${event.typeName}</span>`;
...
function createScheduleListItem(event) {
    ...
    const typeBadge = isCompleted
        ? `<span class="badge" style="background:#FDDDD9; color:#7B1A13; border:1px solid #C0392B; font-size:0.65rem; padding:2px 6px;">${event.typeName}</span>`
        : `<span class="pixel-badge pixel-badge--green" style="font-size: 0.65rem; padding: 2px 6px;">${event.typeName}</span>`;
    const statusBadge = isCompleted
    ...
    const btnAction = isCompleted
        ? `<a href="${event.locationUrl}" target="_blank" rel="noopener noreferrer" class="btn btn-sm" style="background:#C0392B; color:#fff; border:1.5px solid #7B1A13;">
               <span>ARSIP</span>
               <span style="font-family: var(--font-pixel);">↗</span>
           </a>`
        : `<a href="${event.locationUrl}" target="_blank" rel="noopener noreferrer" class="btn btn-outline btn-sm">
               <span>LOKASI</span>
               <span style="font-family: var(--font-pixel);">↗</span>
           </a>`;
        ...
        <div style="display: flex; align-items: center; gap: 18px;">
            ...
            <div>
                <div style="display: flex; gap: 8px; margin-bottom: 4px; align-items: center; flex-wrap: wrap;">
                    ${typeBadge}
                    ${statusBadge}
                    <span style="font-size: 0.82rem; font-weight: 700; color: var(--color-grey);">${event.dayName}, ${event.time}</span>
                </div>
                <h4 style="margin: 0; font-size: 1.3rem; color: ${titleColor};">${event.title}</h4>
                <div style="font-size: 0.88rem; color: var(--color-grey); margin-top: 2px;">📍 ${event.venue}</div>
            </div>
        </div>
        <div>
            ${btnAction}
