...
BBC_onReady(() => {
    try {
        ...
        // Gallery list
        const gallery = Array.isArray(player.gallery)
            ? player.gallery.filter(item => item && (typeof item === 'string' ? item.trim() : (item.url || item.image)))
            : [];
                        ...
                        <div class="pd-gallery-block">
                            ...
                            <div class="pd-gallery-grid">
                                ${gallery.map((item, idx) => {
                                    const photoUrl = typeof item === 'string' ? item.trim() : (item.url || item.image || '').trim();
                                    const photoCaption = (typeof item === 'object' && item && item.caption) ? item.caption : '';
                                    return `
                                <div class="pd-gallery-card" data-gallery-index="${idx}" tabindex="0" role="button" title="${photoCaption || 'Lihat foto aksi'}">
                                    <div class="pd-gallery-img-wrap">
                                        <img src="${photoUrl || fallbackSvg}"
                                             alt="${photoCaption || ('Foto Aksi ' + player.name)}"
                                             loading="lazy"
        ...
        if (lightboxEl && gallery.length > 0) {
            ...
            function showLightboxPhoto(idx) {
                ...
                const pUrl = typeof photo === 'string' ? photo.trim() : (photo.url || photo.image || '').trim();
                const pCaption = (typeof photo === 'object' && photo && photo.caption) ? photo.caption : '';

                if (lightboxImg) {
                    lightboxImg.src = pUrl || fallbackSvg;
                    lightboxImg.onerror = function () {
                        this.onerror = null;
                        this.src = fallbackSvg;
                    };
                }
                if (lightboxCaption) lightboxCaption.textContent = pCaption;
        ...
        console.error('[BBC Player Detail] Error rendering player detail:', err);
        const container = document.getElementById('player-detail-container');
        if (container) {
            container.innerHTML = `
                <div class="pd-empty-state">
                    <span class="pixel-badge pixel-badge--coral" style="margin-bottom: 12px;">STATUS: ERROR</span>
                    <h3>GAGAL MEMUAT PROFIL PEMAIN</h3>
                    <p style="color: var(--color-grey); margin-bottom: 18px;">Terjadi kendala saat memproses data profil atlet ini.</p>
